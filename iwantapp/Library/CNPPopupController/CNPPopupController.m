...
- (instancetype)initWithContents:(NSArray <UIView *> *)contents {
    ...
    if (self) {
        ...
        // [_popupView setContentSize:CGSizeMake(320,2700)];
...
- (void)applyTheme {
    ...
    if (self.theme.popupStyle == CNPPopupStyleBottom) {
        self.popupView.layer.cornerRadius = self.theme.cornerRadius;
    }
...
- (CGPoint)endingPoint {
    ...
    if (self.theme.popupStyle == CNPPopupStyleBottom) {
        center = CGPointMake(self.maskView.center.x, self.maskView.bounds.size.height-(self.popupView.bounds.size.height * 0.5) - 5);
    }
    return center;
}
...
+ (CNPPopupTheme *)menuTheme {
    CNPPopupTheme *menuTheme = [[CNPPopupTheme alloc] init];
    menuTheme.backgroundColor = [UIColor colorWithRed:239.0f/255.0f
                                                green:239.0f/255.0f
                                                 blue:244.0f/255.0f
                                                alpha:1.0f];
    menuTheme.cornerRadius = 4.0f;
    menuTheme.popupContentInsets = UIEdgeInsetsMake(0.0f, 16.0f, -1.0f, 16.0f);
    menuTheme.popupStyle = CNPPopupStyleBottom;
    menuTheme.presentationStyle = CNPPopupPresentationStyleSlideInFromBottom;
    menuTheme.dismissesOppositeDirection = NO;
    menuTheme.maskType = CNPPopupMaskTypeDimmed;
    menuTheme.shouldDismissOnBackgroundTouch = NO;
    menuTheme.movesAboveKeyboard = YES;
    menuTheme.contentVerticalPadding = 0.0f;
    menuTheme.maxPopupWidth = 300.0f;
    menuTheme.animationDuration = 0.3f;
    return menuTheme;
}
